            ...
            if ((its_client == host_->get_client() && is_response)
                ...
                if (e2e_provider_) {
                    if ( !is_service_discovery) {
                        service_t its_service_inner = VSOMEIP_BYTES_TO_WORD(
                                _data[VSOMEIP_SERVICE_POS_MIN], _data[VSOMEIP_SERVICE_POS_MAX]);
                        method_t its_method_inner = VSOMEIP_BYTES_TO_WORD(
                                _data[VSOMEIP_METHOD_POS_MIN], _data[VSOMEIP_METHOD_POS_MAX]);
#ifndef ANDROID
                ...
                if (is_request) {
                    ...
                    if (its_info || is_service_discovery) {
                        if (is_notification && !is_service_discovery) {
                            ...
                            method_t its_method_inner = VSOMEIP_BYTES_TO_WORD(_data[VSOMEIP_METHOD_POS_MIN],
                ...
                if (ep) {
                    if (ep_mgr_impl_->remove_instance(_service, ep.get())) {
                        ...
                        ep->prepare_stop(
                            ...
                            (std::shared_ptr<endpoint> _endpoint_inner,
                            service_t /* _stopped_service_inner */) {
                                if (ep_mgr_impl_->remove_server_endpoint(
                                                _endpoint_inner->get_local_port(),
                                                _endpoint_inner->is_reliable())) {
                                    _endpoint_inner->stop();
                                }
                                {
                                    std::lock_guard<std::mutex> its_lock(callback_counts_mutex_);
                                    if (is_last_stop_callback(its_callback_id)) {
