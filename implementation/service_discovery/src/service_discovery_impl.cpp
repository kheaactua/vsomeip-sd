...
#else
    // Omitting a Linux implementation for now.  The QNX implementation might be
    // modified to use pps, thus the Linux implementation would use Netlink or
    // something.. If the Linux implementation mirrored what is above it should
    // use ionotify, but in any case the interface is never missing on Linux
    return true;
#endif
}
